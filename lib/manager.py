...
RULE_MAIN_DIR = os.path.join(CURRENT_DIR, 'data', 'suri_main')

if not os.path.exists(RULE_MAIN_DIR):
    print('NOT Exist rulesets DIR')
    exit(0)

ClassificationMappingPath = os.path.join(RULE_MAIN_DIR, *['classification.config'])
DISABLE_CONF_PATH = os.path.join(CURRENT_DIR, *['data', 'disable.conf'])
...
class RuleManager:
    ...
    @staticmethod
    def collected_rules_by_dirpath(dirpath=RULE_MAIN_DIR):
...
class RuleXableManager:
    # TODO: 切记不要在配置文件中用 \n 字符。
    def __init__(self, disable_conf=DISABLE_CONF_PATH):
